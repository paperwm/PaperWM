...
function _checkWorkspaces() {
    ...
    for (i = 0; i < Math.max(Main.layoutManager.monitors.length, minimum - 1); i++) {
    ...
    if (lastEmptyIndex > minimum) {
        emptyWorkspaces[minimum - 1] = false;
    }

    // If we don't have an empty workspace at the end, add one
    if (!emptyWorkspaces[emptyWorkspaces.length -1]) {
        log('Apppend empty workspace')
        workspaceManager.append_new_workspace(false, global.get_current_time());
        emptyWorkspaces.push(true);
    }

    let lastIndex = emptyWorkspaces.length - 1;
    lastEmptyIndex = emptyWorkspaces.lastIndexOf(false) + 1;
