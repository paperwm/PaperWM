...
export function registerWindow(metaWindow) {
    ...
    signals.connect(clone, 'notify::position', () => {
        const space = spaces.spaceOfWindow(metaWindow);
        space.zenMode.clonePositionChange(metaWindow);
    });
...
export function showWindowActor(metaWindow) {
    ...
    hideWindowCloneActor(metaWindow);

    const space = spaces.spaceOfWindow(metaWindow);
    if (!space.zenMode.shouldShowWindowActor(metaWindow)) {
        return true;
    }
...
export function showWindowClone(metaWindow) {
    ...
    showWindowCloneActor(metaWindow);
...
/**
 * A window's cloneActor is a rendered actor of a clone (actual actor shown).
 * This methods shows the cloneActor.
 * @param {MetaWindow} metaWindow
 */
export function showWindowCloneActor(metaWindow) {
    const actor = metaWindow.get_compositor_private();
...
/**
 * A window's cloneActor is a rendered actor of a clone (actual actor shown).
 * This method hides the cloneActor.
 * @param {MetaWindow} metaWindow
 */
export function hideWindowCloneActor(metaWindow) {
    const cloneActor = metaWindow.clone?.cloneActor;
    if (cloneActor) {
        cloneActor.hide();
...
class ZenMode {
    ...
    /**
     * Enables ZenMode (hides non-active window actors).
     * @param {MetaWindow} metaWindow
     * @param {Boolean} later
     */
    ...
    /**
     * Disables ZenMode (shows all window actors).
     */
    disableZenMode() {
     ...
     * @returns Boolean
     */
    shouldShowWindowActor(metaWindow) {
        if (
            this.isZenMode &&
     ...
     * @returns
     */
    clonePositionChange(metaWindow) {
        if (
            this.isZenMode &&
    ...
    hideWindowActors(...exclude) {
        ...
        if (this.isZenMode) {
            ...
            .forEach(w => {
                ...
                if (!this.space.isPlaceable(w)) {
                    hideWindowCloneActor(w);
                }
            });
        }
    }
