...
The `wm_class` of a window can be looked up by clicking <kbd>Super</kbd><kbd>Insert</kbd> and then checking the value of `metaWindow.wm_class` in emacs or looking glass. X11 users can use `xprop | grep WM_CLASS` at the command line then clicking on the window.
